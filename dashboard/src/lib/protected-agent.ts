...
// Tool definitions - same as unprotected agent
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        patient_id: {
          type: 'string',
          description: 'The patient ID to send report for',
        },
      },
      required: ['email', 'patient_id'],
    },
  },
];
...
function handleToolCall(name: string, input: Record<string, string>): string {
  switch (name) {
    ...
    case 'sendPatientReport': {
      ...
      // Even if attacker gets the email sent, it only contains tokens!
      return JSON.stringify({
        success: true,
        message: `Report sent to ${input.email}`,
        data_sent: {
          patient_name: patient.patient_name_token,
          ssn: patient.ssn_token,
          diagnosis: patient.diagnosis_token,
          medications: patient.medications_token,
        },
        _notice: 'Only Skyflow tokens were transmitted. No actual PII exposed.',
      });
    }
    default:
